...
// size we will use for the camera and renderer. Here we are using the client's window entirely. Keep in mind that since these values are captured once when first
...
// scene
// we create a 3D scene. This is where we will add our meshes and cameras. You can think of this as the 3D world/space we will work in.
...
// finally we create a mesh. A mesh takes in a geometry (a shape, in this case our box) and a material to apply to that geometry, in this case our red material.
const mesh = new THREE.Mesh(geometry, material);

// add mesh to scene. Creating a mesh (a geometry with a material applied to it) is not enough, we also need to add that mesh into our world.
scene.add(mesh);

// camera
// a camera is a special object that we can add into a scene that defines some properties related to how we see the world.
...
// change by sliding that number from one extreme to the other based on that information.
// the 1 and 1000 values are, I believe, the clipping ranges. Meaning the depth range you want to capture. If the units were meters, you can think of these as
